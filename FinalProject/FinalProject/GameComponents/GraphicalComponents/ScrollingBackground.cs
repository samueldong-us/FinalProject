...
namespace FinalProject.GameComponents
{
    internal class ScrollingBackground
    {
        ...
        {
            if (currentY > 4096 - GameScreen.Visible.Height)
            {
                spriteBatch.Draw(background, new Rectangle(GameScreen.Visible.Left, GameScreen
                    .Visible.Bottom - 4096 + (int)currentY, GameScreen
                    .Visible.Width, 4096 - (int)currentY), new Rectangle(currentRow * 1024, 0, 1024, 4096 - (int)currentY), Color.White);

                spriteBatch.Draw(background, new Rectangle(GameScreen.Visible.Left, GameScreen
                    .Visible.Top, GameScreen
                    .Visible.Width, GameScreen.Visible.Height - 4096 + (int)currentY), new Rectangle(nextRow * 1024, 8192 - (int)currentY - GameScreen.Visible.Height, 1024, GameScreen.Visible.Height - 4096 + (int)currentY), Color.White);
            }
            else
            {
                spriteBatch.Draw(background, GameScreen.Visible, new Rectangle(currentRow * 1024, 4096 - (int)currentY - GameScreen.Visible.Height, 1024, GameScreen.Visible.Height), Color.White);
            }
        ...
        public void Update(float secondsPassed)
        {
            ...
            for (int i = planetInfo.Count - 1; i >= 0; i--)
            {
                ...
                if (planetInfo[i].Y > GameScreen.Bounds.Bottom + 700)
                {
        ...
        private void AddPlanet()
        {
            ...
            float x = (float)(GameMain.RNG.NextDouble() * GameScreen.Bounds.Width + GameScreen.Bounds.Left);
            float rotSpeed = RandomRotationSpeed();
            Vector4 info = new Vector4(x, GameScreen.Bounds.Top - 700, (float)(Math.PI * 2 * GameMain.RNG.NextDouble()), rotSpeed);
            planetInfo.Add(info);
        }

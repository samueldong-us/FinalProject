    ...
    internal class ComponentBehaviorCatmullRom : DrawableComponent
    {
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            for (float p = 0; p < timeToFinish; p += .1f)
            {
                ...
                spriteBatch.Draw(UtilitiesGraphics.PlainTexture, new Rectangle((int)point.X - 2, (int)point.Y - 2, 4, 4), Color.Blue);
            }
            for (int index = 0; index < pointList.Count; index++)
            {
                Vector2 point = GetKeyPointAt(index);
                spriteBatch.Draw(UtilitiesGraphics.PlainTexture, new Rectangle((int)point.X - 4, (int)point.Y - 4, 8, 8), Color.Green);
            }
        }

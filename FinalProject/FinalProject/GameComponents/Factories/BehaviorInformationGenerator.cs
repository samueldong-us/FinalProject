...
namespace FinalProject.GameComponents
{
    internal static class BehaviorInformationGenerator
    {
        ...
        public static Vector4 RandomSpawnAndSwitchPosition()
        {
            ...
            return new Vector4(spawnPosition.X, spawnPosition.Y, MathHelper.Clamp(switchPosition.X, GameScreen.Bounds.Left + 100, GameScreen.Bounds.Right - 100), switchPosition.Y);
        }
        ...
        {
            return (float)(GameMain.RNG.NextDouble() * GameScreen.Bounds.Width * .8 + GameScreen.Bounds.Left + GameScreen.Bounds.Width * .1);
        }

        private static Vector2 RandomPointInQuadrant(int row, int column)
        {
            int originX = GameScreen.Bounds.Left - 550;
            int originY = GameScreen.Bounds.Top - 250;
